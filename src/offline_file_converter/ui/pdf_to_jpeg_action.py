...
from offline_file_converter.conversion.pdf_to_jpeg import convert_pdf_files_to_jpeg
...
class PdfToJpegWorker(QRunnable):
    def __init__(
        self,
        paths: tuple[Path, ...],
        dpi: int,
...
class PdfToJpegAction(QFrame):
    ...
    def __init__(self, strings: dict[str, str]) -> None:
        ...
        self._running = False
        self._worker: PdfToJpegWorker | None = None

        self.setObjectName("pdfToJpegAction")
        self.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)

        self._button_shell = QFrame()
        self._button_shell.setObjectName("pdfToJpegButton")
        ...
        self._run_button.setObjectName("pdfToJpegRunButton")
        ...
        self._options_button.setObjectName("pdfToJpegOptionsButton")
    ...
    def _button_text(self) -> str:
        return self._strings["pdf_to_jpeg"].format(extension=self._extension)
    ...
    def _start_conversion(self) -> None:
        if self._running or not self._pdf_paths:
            return
    ...
    def _conversion_succeeded(self, outputs: list[Path]) -> None:
        ...
        tooltip = self._strings["jpeg_conversion_success"].format(count=len(outputs))
        if folders:
            tooltip = f"{tooltip}\n" + "\n".join(folders)
    ...
    def _conversion_failed(self, details: str) -> None:
        tooltip = f"{self._strings['jpeg_conversion_failed']}\n{details}"
        self._finish_conversion()
