...
from offline_file_converter.conversion.pdf_to_png import convert_pdf_files_to_png
...
class PdfToPngWorker(QRunnable):
    def __init__(
        self,
        paths: tuple[Path, ...],
        dpi: int,
...
class PdfToPngAction(QFrame):
    def __init__(self, strings: dict[str, str]) -> None:
        ...
        self._running = False
        self._worker: PdfToPngWorker | None = None

        self.setObjectName("pdfToPngAction")
        self.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)

        self._button_shell = QFrame()
        self._button_shell.setObjectName("pdfToPngButton")
        ...
        self._run_button = QPushButton(self._strings["pdf_to_png"])
        self._run_button.setObjectName("pdfToPngRunButton")
    ...
    def _conversion_succeeded(self, outputs: list[Path]) -> None:
        ...
        tooltip = self._strings["conversion_success"].format(count=len(outputs))
        if folders:
            tooltip = f"{tooltip}\n" + "\n".join(folders)
    ...
    def _conversion_failed(self, details: str) -> None:
        tooltip = f"{self._strings['conversion_failed']}\n{details}"
        self._finish_conversion()
    ...
    def _finish_conversion(self) -> None:
        self._running = False
        self._worker = None
        self._run_button.setText(self._strings["pdf_to_png"])
