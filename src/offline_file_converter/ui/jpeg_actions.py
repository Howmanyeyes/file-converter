...
from offline_file_converter.conversion.jpeg_compress import (
    compress_jpeg_file,
    compress_jpeg_with_high_quality,
)
from offline_file_converter.conversion.jpeg_merge import merge_jpeg_files
from offline_file_converter.conversion.jpeg_to_pdf import (
    convert_jpeg_files_to_pdf,
)
from offline_file_converter.conversion.jpeg_to_png import (
    convert_jpeg_files_to_png,
)
...
class JpegAction(PdfToolAction):
    def __init__(
        self,
        ...
        *,
        ...
        self._jpeg_paths: tuple[Path, ...] = ()
        if utility:
            self.setObjectName("jpegToolAction")
            self._button_shell.setObjectName("jpegToolButton")
            self._run_button.setObjectName("jpegToolRunButton")
        else:
            self.setObjectName("jpegConversionAction")
            self._button_shell.setObjectName("jpegConversionButton")
            self._run_button.setObjectName("jpegConversionRunButton")

    @Slot(object)
    def set_files(self, selected_files: tuple[SelectedFile, ...]) -> None:
        self._jpeg_paths = tuple(
            item.path
            for item in selected_files
            if item.detected_format == "jpeg"
        )
        if not self._running:
            self._set_status(None)
        self.setVisible(bool(self._jpeg_paths))
        self._update_availability()

    def _update_availability(self) -> None:
        self._run_button.setEnabled(
            bool(self._jpeg_paths) and not self._running
        )


class JpegToPngAction(JpegAction):
    def __init__(self, strings: dict[str, str]) -> None:
        super().__init__(
            strings,
            "jpeg_to_png",
            "jpeg_png_conversion_success",
            "jpeg_png_conversion_failed",
            utility=False,
        )

    def _activate(self) -> None:
        if not self._jpeg_paths:
            return
        paths = self._jpeg_paths
        output_directory = self._output_directory
        self._start_operation(
            lambda progress: convert_jpeg_files_to_png(
                paths,
                output_directory,
                progress,
            )
...
class JpegToPdfAction(JpegAction):
    def __init__(self, strings: dict[str, str]) -> None:
        super().__init__(
            strings,
            "jpeg_to_pdf",
            "jpeg_pdf_conversion_success",
            "jpeg_pdf_conversion_failed",
            utility=False,
        )
    ...
    def _activate(self) -> None:
        ...
        output_directory = self._output_directory
        self._start_operation(
            lambda progress: convert_jpeg_files_to_pdf(
                paths,
                output_directory,
                progress,
            )
...
class JpegMergeAction(JpegAction):
    def __init__(self, strings: dict[str, str]) -> None:
        super().__init__(
            strings,
            "jpeg_merge",
    ...
    def _activate(self) -> None:
        if len(self._jpeg_paths) < 2:
            return
        paths = self._jpeg_paths
        output_directory = self._output_directory
        self._start_operation(
            lambda progress: merge_jpeg_files(
                paths,
                output_directory,
                progress,
            )
    ...
    def _update_availability(self) -> None:
        can_merge = len(self._jpeg_paths) >= 2 and not self._running
        self._run_button.setEnabled(can_merge)
        self._run_button.setToolTip(
            "" if can_merge else self._strings["jpeg_merge_requires_multiple"]
        )


class JpegCompressAction(JpegAction):
    def __init__(self, strings: dict[str, str]) -> None:
        super().__init__(
            strings,
            "jpeg_compress",
            "jpeg_compress_success",
            "jpeg_compress_failed",
            utility=True,
        )
        self._requested_target_size = 0
        self._high_quality_operation = False

    def _activate(self) -> None:
        if not self._jpeg_paths:
            return

        dialog = JpegCompressDialog(self._jpeg_paths, self._strings, self)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return

        source_path = dialog.selected_path
        output_directory = self._output_directory
        self._high_quality_operation = dialog.high_quality
        if self._high_quality_operation:
            self._start_operation(
                lambda progress: compress_jpeg_with_high_quality(
                    source_path,
                    output_directory,
                    progress,
                )
        ...
        self._start_operation(
            lambda progress: compress_jpeg_file(
                source_path,
                target_size_bytes,
                output_directory,
                progress,
    ...
    def _operation_succeeded(self, outputs: list[Path]) -> None:
        if (
            not self._high_quality_operation
            and outputs
            and outputs[0].stat().st_size > self._requested_target_size
        ):
            tooltip = self._strings["jpeg_compress_minimum_success"].format(
...
class JpegCompressDialog(QDialog):
    def __init__(
        self,
        paths: tuple[Path, ...],
        strings: dict[str, str],
        ...
        self.setWindowTitle(self._strings["jpeg_compress_dialog_title"])
        ...
        title = QLabel(self._strings["jpeg_compress_dialog_title"])
        title.setObjectName("pdfToolTitle")
        description = QLabel(
            self._strings["jpeg_compress_dialog_description"]
        )
        ...
        source_label = QLabel(self._strings["jpeg_source_file"])
        ...
        target_label = QLabel(self._strings["jpeg_target_size"])
        ...
        hint = QLabel(self._strings["jpeg_compress_quality_warning"])
        hint.setObjectName("pdfToolHint")
        ...
        high_quality_button = buttons.addButton(
            self._strings["jpeg_compress_high_quality"],
            QDialogButtonBox.ButtonRole.ActionRole,
        )
        high_quality_button.setObjectName("pdfToolLosslessButton")
        confirm_button.setText(self._strings["jpeg_compress_start"])
        cancel_button.setText(self._strings["cancel"])
        high_quality_button.clicked.connect(self._accept_high_quality)
    ...
    def _accept_high_quality(self) -> None:
    ...
    def _sync_source_information(self) -> None:
        ...
        self._source_size_label.setText(
            self._strings["jpeg_source_size"].format(
